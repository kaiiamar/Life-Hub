...
function renderPage(page){if(page==='dashboard')renderDashboard();if(page==='roadmap')renderRoadmap();if(page==='goals')renderGoals();if(page==='habits')renderHabits();if(page==='workout')renderWorkout();if(page==='finance')renderFinance('overview');if(page==='metrics')renderMetrics();if(page==='review')renderReview();if(page==='weekly')renderWeeklyPlan();if(page==='insights')renderInsights();if(page==='projects')renderProjects();if(page==='relationships')renderRelationships();if(page==='gratitude')renderGratitude();if(page==='watchlist')renderWatchlist();if(page==='wishlist')renderWishlist();if(page==='skincare')renderSkincare();if(page==='tasks')renderTasksArchive()}
